...
module tSlot(add=true)
{
	if (add==true)
		{	
			translate([-0.5*tSlotWidth,-tol,-thickness/2]) cube([tSlotWidth, tSlotLength + 2* tol, 2* thickness ]);
			translate([-nutWidth/2,tSlotLength/2,-thickness/2]) cube([nutWidth,nutThickness, 2*thickness]);
		...
		rotate(rotation)
			for (n=[1: number])
			{
				translate([(-0.5 + n)*length/number ,0,0])
				{
					...
					if (tab) translate([-3* thickness,-depth,0]) tab(add);
					if (tSlot) tSlot(add);
					if (tab) translate([3* thickness,-depth,0]) tab(add);
				}	
			}	
